...
export interface XlviLoaderProps extends LoaderSizeProps {
  /** @deprecated Typo preserved for backwards compatibility — use background in future versions */
  backgound?: string;
...
const XlviLoader = ({
  ...
  if (typeof boxColors === "string") {
    colorsToFill.push(boxColors === "" ? "#333" : boxColors);
  } else {
    const colors = boxColors.length === 0 ? ["#333"] : boxColors;
    for (let i = 0; i < 3; i += 1) {
      colorsToFill.push(
