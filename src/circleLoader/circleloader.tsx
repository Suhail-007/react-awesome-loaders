...
export interface CircleLoaderProps extends LoaderSizeProps {
  meshColor?: string;
  lightColor?: string;
...
const CircleLoader = ({
  ...
  useEffect(() => {
    const canvas = document.querySelector<HTMLCanvasElement>("." + className);
    if (!canvas) return;
    ...
    let width = sizeCanvas,
      ...
      shape = (inner = 13, outer = 20, height = 6, segments = 60) => {
        ...
        let geometry = new THREE.ExtrudeGeometry(arc, {
          ...
          proxy = new Proxy<{ z: number | null; s: number | null }>(
            {
              z: null,
              s: null,
            },
            {
              set(target, key: "z" | "s", value: number | null) {
                target[key] = value;
                if (target.z !== null) {
